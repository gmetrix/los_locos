...
public class JoystickView extends View implements Controller {
	...
	private DriveControl drive_ctrl = DriveControl.INSTANCE;
	...
	/*
	 * Sett the radius of the puck to the provided radius, in pixels
	 * */
	...
	/*
	 * Reset the puck's position to the middle of the wheel
	 * */
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom){
		...
		if(this.getMeasuredWidth() > this.getMeasuredHeight()/2){
	...
	/*
	 * From a provided Point containing the puck's current position returns a Point containing
	 * a valid coordinate for use with the Drivecontrol's joystick area
	 *
	 *@param current_position
	 *@return a point containing the clipped coordinates
	 */
	...
	/*
	 * Get a point corrected for this Views position
	 * */
		...
		switch(event.getActionMasked()){
				...
				if(mEnabled){
					if(this.robot != null && this.robot.isConnected()){
						...
						if(puck.getBounds().contains(local_point.x, local_point.y)){
							...
							this.drive_ctrl.setSpeedScale(this.speed);
							this.drive_ctrl.startDriving(this.getContext(), DriveControl.JOY_STICK);
						}
			...
			case MotionEvent.ACTION_MOVE:
				if(mEnabled && draggingPuck && draggingPuckPointerId == pointer_id){
					...
					this.drive_ctrl.driveJoyStick(drive_coord.x, drive_coord.y);
